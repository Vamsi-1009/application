...
- The whole visual shell (CSS + JS + hero + canvas) is applied uniformly across all 19 pages by a one-off "re-sheller" Node script (scratch tool, not committed): it extracts each page's `<title>`, `<nav class="rail">`, and `<main>` inner content, then re-wraps them in the shared new template — so restyling never touches the prose. To restyle again, either re-run a similar re-sheller or hand-edit; there is no committed build step by design (plain dependency-free HTML).
